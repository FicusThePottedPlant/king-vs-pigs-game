...
LEVELS = 9
left, right, up = False, False, False  # there is some flags to moving on level
        ...
        if self.text_x <= mouse_pos[0] <= self.text_x + self.text_w and \
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if not windows_behaviour.get(self.button_text) and display_levels:
                    """There is ivent of choosing level"""
                    game = Game(self.button_text)
...
class Game:
    ...
    def update(self, left, right, up, pygame_event,what):
        if what:
            self.player.update(left, right, up, platforms)
        else:
...
if __name__ == '__main__':
    ...
    while running:
        ...
        if is_playing:
            game.update(left, right, up, None,True)
        all_window_render()
